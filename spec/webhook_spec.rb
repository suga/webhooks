...
describe Webhook do
  ...
  context 'parser_request_to' do
    it 'When we have 4 requests in the file' do
      webhook = Webhook.new(log_file:@file_temp_log.read)
      expect(webhook.parser_request_to.count).to eq(4)
    end
  end

  context 'parser_response_status' do
    it 'When we have 5 responses in the file' do
      webhook = Webhook.new(log_file:@file_temp_log.read)
      expect(webhook.parser_response_status.count).to eq(5)
    end
  end
